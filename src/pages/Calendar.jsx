  ...
  const handleDragEnd = (event) => {
    ...
    try {
      ...
      const updatedTasks = tasks.map(t => 
        t.id === taskId 
          ? { ...t, dueDate: format(newDate, 'yyyy-MM-dd'), updatedAt: new Date().toISOString() }
          : t
      )
      ...
      toast.success(`Task "${task.title}" moved to ${format(newDate, 'MMM d, yyyy')}`)
    } catch (error) {
    ...
    return (
      <motion.div
        ...
        className={`
          ...
          ${isOver ? 'bg-primary-100 border-primary-300 shadow-inner' : ''}
        `}
    ...
    >
      <div className="min-h-screen bg-gradient-to-br from-surface-50 via-surface-100 to-primary-50 dark:from-surface-900 dark:via-surface-800 dark:to-primary-900">
        ...
        <DragOverlay>
          {draggedTask ? (
            <div className={`
              text-xs p-1 rounded truncate text-white font-medium shadow-lg
              ${getPriorityColor(draggedTask.priority)}
            `}>
              {draggedTask.title}
            </div>
          ) : null}
